...
# 2 - Total de salário dos homens;
# 3 - Total de salário das mulheres;
# 4 - Listar todos os funcionários;
# 5 - Total de salário por departamento;
# -> Exibir todos os departamentos
# -> Escolha um departamento: Treinador
# -> Total de salário do departamento Treinador é 9000
# 0 - Sair do programa;
...
def exibir_departamentos():
    departamentos = list(
        set(map(lambda funcionario: funcionario['departamento'], funcionarios)))
...
def salvar_arquivo(funcionario: dict):
    with open('funcionarios.txt', 'a') as arquivo:
        arquivo.write(f"{funcionario['nome']},{funcionario['sexo']},{funcionario['salario']},{funcionario['departamento']}\n")
def totalizar(**parametros):
    filtrado = filter(lambda funcionario: funcionario[parametros['campo']] == parametros['valor'], funcionarios)
...
def main():
    ...
    while True:
        ...
        if opcao == 0:
            ...
            print(f'Soma do salário do departamento {departamento} {total}')
        else:
